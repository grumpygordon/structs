/*
 *  Wavelet tree with low memory usage.
 *  Supports several orthogonal queries in O(log n)
 *  Paper: Wavelet Trees for Competitive Programming, ioiconf16
 *
 *  This implementation with popcount is inspired by anta's WaveletMatrix.
 */
...
public:
    ...
    Wavelet_Matrix(vector<T> v): n(v.size()), data(height, n){
        build(move(v));
    }
